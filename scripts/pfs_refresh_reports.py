...
def main():
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('output',type=Path);p.add_argument('--workers',type=int,default=4);p.add_argument('--watch',action='store_true');p.add_argument('--codes',nargs='+');a=p.parse_args();out=a.output.resolve()
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=a.workers) as pool:
        ...
        while True:
            for dest in (out/'results').glob('P*'):
                if a.codes and dest.name not in a.codes:continue
            ...
            (out/('report_refresh_subset_status.json' if a.codes else 'report_refresh_status.json')).write_text(json.dumps(summary,indent=2)+'\n')
            batch=json.loads((out/'batch_status.json').read_text())
            expected=len(a.codes) if a.codes else batch['requested']
            if not futures and (not a.watch or (batch['reported']==batch['requested'] and len(attempted)>=expected)):break
            time.sleep(3)
